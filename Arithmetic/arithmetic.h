...
public:
  ...
  long double binary_to_decimal(string fraction);
  ...
  string  decimal_to_binary(long double fraction);
  ...
  void calc_pro(const vector<unsigned char>& msg);
  void Read_Alph();
  void Write_Alph();
private:
  // Data members here ... 
	vector<pair<unsigned char, double>> Alphapet;
	vector<pair<unsigned char, double>>CDF;
